    ...
    switch (engine) {
        ...
        case "xtts": {
            if (!speakerWav || !speakerWav.trim()) {
                console.warn("[TTS] XTTS requested but no speakerWav sample provided. Using ElevenLabs voice...");
                const resolvedVoice = (voiceId && voiceId.length > 15) ? voiceId : "21m00Tcm4TlvDq8ikWAM";
                return generateVoiceover({ ...options, engine: "elevenlabs", voiceId: resolvedVoice });
            }
