...
function ScriptTab({ doc, onRefresh }: { doc: any; onRefresh: () => void }) {
    ...
    return (
        <div className="space-y-3">
            ...
            {/* Voice & Narration Audition */}
            <VoiceAudition doc={doc} scriptText={script} />
        </div>
    );
}

/* ────── Voice Audition Component ────── */
function VoiceAudition({ doc, scriptText }: { doc: any; scriptText: string }) {
    const [expanded, setExpanded] = useState(false);
    const [ttsEngine, setTtsEngine] = useState(doc.ttsEngine || "elevenlabs");
    const [narratorStyle, setNarratorStyle] = useState(doc.narratorStyle || "documentary");
    ...
    // Load voices when engine changes or panel opens
    useEffect(() => {
        if (!expanded) return;
        ...
        fetch(`/api/voiceover/voices?engine=${ttsEngine}`)
            ...
            .then(data => {
                ...
                if (data.voices?.length > 0 && !ttsVoiceId) {
    ...
    const saveSetting = async (key: string, value: string) => {
        setSaving(true);
        await fetch(`/api/documentary/${doc.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
    ...
    // Audition with script text — TTS first ~200 chars
    const auditionWithScript = async () => {
        if (audioRef) { audioRef.pause(); }
        setAuditioning(true);

        const sampleText = scriptText
            .replace(/\[VISUAL:[^\]]*\]/g, "") // strip visual markers
            .trim()
            .substring(0, 200);

        if (!sampleText) {
            alert("No script text to audition. Write or generate a script first.");
        ...
        try {
            ...
            if (!res.ok) {
                ...
                alert(`Audition failed: ${err.error || res.statusText}`);
                setAuditioning(false);
                return;
            }
            ...
            audio.onended = () => { setAuditioning(false); URL.revokeObjectURL(url); };
            audio.onerror = () => { setAuditioning(false); URL.revokeObjectURL(url); };
            audio.play();
            setAudioRef(audio);
        } catch {
            setAuditioning(false);
    ...
    const styleOptions = [
        { value: "sleep", label: "Sleep / Calm", desc: "0.85×" },
        { value: "documentary", label: "Documentary", desc: "0.92×" },
        { value: "dramatic", label: "Dramatic", desc: "0.95×" },
        { value: "energetic", label: "Energetic", desc: "1.1×" },
        { value: "conversational", label: "Conversational", desc: "1.0×" },
    ];

    const estCost = ttsEngine === "elevenlabs" ? `~$${(0.30 * 200 / 1000).toFixed(2)}` : "Free";
    const selectedVoice = voices.find((v: any) => v.id === ttsVoiceId);
        ...
        <div className="bg-gray-900/50 border border-gray-800 rounded-xl overflow-hidden">
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-white">Voice & Narration</span>
                    {selectedVoice && (
            ...
            {expanded && (
                <div className="px-4 pb-4 space-y-4 border-t border-gray-800">
                    {/* Engine Selector */}
                    <div className="pt-3">
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Engine</p>
                        <div className="flex gap-2">
                            {engineOptions.map(opt => (
                                <button
                                    key={opt.value}
                                    onClick={() => { setTtsEngine(opt.value); saveSetting("ttsEngine", opt.value); }}
                                    className={cn(
                                        "flex-1 text-left p-2.5 rounded-lg border transition-all text-xs",
                                        ttsEngine === opt.value
                    ...
                    {/* Style Selector */}
                    <div>
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Narrator Style</p>
                        <div className="flex flex-wrap gap-1.5">
                            {styleOptions.map(opt => (
                                <button
                                    key={opt.value}
                                    onClick={() => { setNarratorStyle(opt.value); saveSetting("narratorStyle", opt.value); }}
                                    className={cn(
                                ...
                                >
                                    {opt.label} <span className="text-gray-500">{opt.desc}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">
                                Voices {loadingVoices && <Loader2 className="w-3 h-3 inline animate-spin ml-1" />}
                            </p>
                            {(playingPreview || auditioning) && (
                                <button onClick={stopAudio} className="flex items-center gap-1 text-xs text-red-400 hover:text-red-300">
                        ...
                        {voices.length > 0 ? (
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[240px] overflow-y-auto pr-1">
                                {voices.map((v: any) => (
                                    <div
                                        key={v.id}
                                        className={cn(
                                            "relative group p-2.5 rounded-lg border transition-all cursor-pointer",
                                            ttsVoiceId === v.id
                                                ? "border-violet-500/50 bg-violet-500/10 ring-1 ring-violet-500/30"
                                                : "border-gray-700 bg-gray-800/50 hover:bg-gray-800 hover:border-gray-600"
                                        )}
                                        onClick={() => { setTtsVoiceId(v.id); saveSetting("ttsVoiceId", v.id); }}
                                    >
                                        <div className="flex items-start justify-between">
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {v.category && (
                                                    <p className="text-[10px] text-gray-500 mt-0.5">{v.category}</p>
                                                )}
                                                {v.description && (
                                                    <p className="text-[10px] text-gray-600 truncate mt-0.5">{v.description}</p>
                                                )}
                                            </div>
                            ...
                            <p className="text-xs text-gray-600 italic py-3">No voices available. Check API key in Admin → API Keys.</p>
                        ) : null}
                    </div>

                    {/* Audition with Script Button */}
                    {scriptText && ttsVoiceId && (
                        <div className="flex items-center gap-3 pt-1">
                            <button
                                onClick={auditionWithScript}
                                disabled={auditioning}
                                className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-medium bg-violet-600 hover:bg-violet-500 text-white disabled:opacity-50 transition-colors"
                            >
                                {auditioning ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Mic className="w-3.5 h-3.5" />}
                                {auditioning ? "Generating..." : "Audition with Script"}
                            </button>
                            <span className="text-[10px] text-gray-500">
                                Reads first ~200 chars of your script • Est. cost: {estCost}
                            </span>
                        </div>
                    )}
                </div>
